...
<xsl:template match="/">
    <xsl:apply-templates select="//c:program[@id=$progid]/c:cores" />
</xsl:template>

<xsl:template match="c:core">
	<div class="acalog-program-core-{count(ancestor::c:core)}">
		<xsl:if test="a:title">
			<h2><xsl:value-of select="a:title" /></h2>
		</xsl:if>
		<xsl:apply-templates select="a:content" />
		<xsl:if test="c:courses">
		<ul>
...
<xsl:template match="c:children">
    ...
    <xsl:apply-templates select="c:core" />
</xsl:template>
...
<xsl:template match="c:permalink">
	<a href="{php:function('acalog_get_url',string(@link-id),string(@to),$catoid)}" target="_blank"><xsl:apply-templates select="*|node()"/></a>
</xsl:template>
	
<xsl:template match="c:course">
	<xsl:param name="courseid" select="@id"/>
	<li>
		<a href="{php:function('acalog_get_url',string(@child-of),string($courseid),$catoid)}" target="_blank"><xsl:value-of select="a:title" /></a> <xsl:value-of select="//c:cores/c:core/c:courses/c:adhoc[@course=$courseid]/a:content" />
    </li>
</xsl:template>
	
<xsl:template match="c:adhoc"></xsl:template>
