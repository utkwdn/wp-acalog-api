...
<xsl:template match="/">
	<h3><xsl:value-of select="//c:course[@id=$courseid]/a:title" /></h3>
    
    <xsl:for-each select="//c:course[@id=$courseid]/a:content/c:field">
    	<xsl:if test="c:field/a:title = 'credit hours'">
        	<xsl:variable name="fieldid" select="@type" />
            <xsl:variable name="credits" select="//c:course[@id=$courseid]/a:content/c:field[@type=$fieldid]/c:data" />
            <p><strong><xsl:choose>
